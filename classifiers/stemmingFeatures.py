...
def add_feature(database, featureName):
    #loop cursor
    cursor = database.cursor()
    #insertion cursor
    set_cursor = database.cursor()
    make_sure_feature_in_db(cursor, featureName)
    ...
    fulltext_no_repeats_query = "SELECT c.CommentID, c.CommentText, a.FullText, a.URL FROM ArticleText a, Comments c, Features f WHERE c.ArticleURL=a.URL AND c.CommentID = f.CommentID AND f.featureName IS NULL"
    for c_id, c_text, a_text, a_url in cursor.execute(fulltext_no_repeats_query):

        c_vec = word_vec(c_text)
        a_vec = word_vec(a_text, url=a_url, m_dict=metadata)
...
def word_vec(text, url=None, m_dict=None):
	if url != None:
		if url in m_dict:
			return m_dict[url]

    stripped = [word.strip(string.punctuation).lower() for word in text.replace("<br/>", " ").split()]
    for word in 
